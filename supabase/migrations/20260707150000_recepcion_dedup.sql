-- ============================================================================
-- Migración: recepcionar_factura tolera códigos de proveedor repetidos.
-- Si una factura repite el mismo supplier_code en varias líneas (o el código ya
-- existe para ese proveedor), se REUTILIZA el mismo producto y se SUMA el stock,
-- en vez de intentar crear dos productos con el mismo internal_code (que provocaba
-- unique_violation / 409). Además, no se registra mapeo supplier_product para
-- códigos vacíos (evita un mapeo espurio con supplier_code = '').
-- Contrato: docs/superpowers/specs/2026-07-07-mapeo-proveedor-producto-design.md
-- ============================================================================
...
begin
  ...
  for ln in select * from jsonb_array_elements(p_lines) loop
    ...
    -- Reutilizar el producto ya mapeado a este código de proveedor. Incluye líneas
    -- anteriores de ESTA misma factura (cuyo mapeo ya quedó en supplier_product) y
    -- códigos que ya existían para el proveedor: evita crear dos productos con el
    -- mismo internal_code. Solo aplica a códigos no vacíos.
    ...
    -- Crear producto nuevo si aún no hay, con código interno {seq}-{código proveedor}
    ...
    -- Recordar mapeo proveedor→código→producto + último costo (solo con código no vacío)
    ...
    -- Sumar stock en la sucursal (si el mismo producto aparece en varias líneas, suma)
