  ...
  schema: {
    ...
    properties: {
      proveedor: { type: "object", additionalProperties: false, required: ["razon_social", "rut", "giro", "direccion"],
        properties: { razon_social: { type: "string" },
          rut: { type: "string", description: "RUT del proveedor emisor tal cual aparece impreso, con puntos y guión (ej: 78.964.380-6). Suele estar en un recuadro-timbre destacado (frecuentemente en rojo) junto al folio, con el formato 'R.U.T. <número>'. Obligatorio: no lo dejes vacío si aparece en el documento." },
    ...
    const resp = await fetchWithTimeout("https://api.openai.com/v1/responses", {
      ...
      body: JSON.stringify({
        ...
        input: [{ role: "user", content: [
          { type: "input_text", text: "Extrae los datos de esta factura de compra chilena en el formato indicado. Montos en pesos (enteros, sin separadores). El proveedor es el emisor. La fecha del documento suele venir impresa como dd/mm/yyyy: conviértela y devuélvela SIEMPRE en formato ISO 8601 YYYY-MM-DD (ej: 02/07/2026 -> 2026-07-02). El proveedor es el emisor: extrae también su giro y su dirección (déjalos vacíos si no aparecen). El RUT del emisor (proveedor) y el folio/número del documento suelen aparecer JUNTOS en un recuadro-timbre destacado (frecuentemente en rojo) con el formato 'R.U.T. <número con puntos y guión>', 'FACTURA ELECTRONICA' (o el tipo de documento) y 'N° <folio>'. Extrae SIEMPRE el RUT del emisor (formato chileno, con puntos y dígito verificador, ej. 78.964.380-6) y el folio (el número que sigue a 'N°', ej. 59763). El folio y el RUT del proveedor son obligatorios: no los dejes vacíos si aparecen en el documento. El tipo de documento (documento.tipo) es el texto de ese recuadro (ej. 'Factura Electrónica')." },
